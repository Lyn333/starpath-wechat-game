...
class ForestTrailMiniGame {
  constructor(canvas, levels) {
    ...
    this.renderer = new SingleBoardRenderer(canvas); this.gridSize = "6x6"; this.difficulty = "easy"; this.mode = "standard"; this.dragCell = null; this.startedAt = Date.now(); this.current = this.pickRandom(); this.start(this.current); this.sound.startBackgroundMusic(); this.leaderboard.initialize().finally(() => this.render());
  }
  ...
  start(level) {
    ...
    this.unsubscribe?.(); this.unsubscribe = this.engine.subscribe((snapshot) => { if (snapshot.status === "completed" && !this.wasCompleted) { this.wasCompleted = true; this.completionSummary = this.progress.markComplete(level, { moves: snapshot.moves, elapsedMs: Date.now() - this.startedAt }); this.sound.complete(); this.leaderboard.submitCompletion(level, this.completionSummary.current).finally(() => this.render()); } this.render(snapshot); });
  }
  view(snapshot = this.engine.getSnapshot()) { return { daily: this.mode === "daily", dailyComplete: this.mode === "daily" && this.progress.isDailyComplete(this.current), gridSize: this.gridSize, difficulty: this.difficulty, difficultyLabel: LABELS[this.difficulty], sound: this.progress.soundEnabled(), points: snapshot.status === "completed" ? Math.max(10, Math.round(1200 / Math.max(1, snapshot.moves))) : 0, time: formatTime(Date.now() - this.startedAt), completion: this.completionSummary || this.progress.getCompletionSummary(this.current), completionVisible: snapshot.status === "completed" && !this.completionDismissed, rankings: this.leaderboard.status() }; }
  ...
  handleStart(event) {
    ...
    if (this.engine.getSnapshot().status === "completed") { if (this.renderer.hit(controls.close, point)) { this.completionDismissed = true; return this.render(); } if (this.renderer.hit(controls.next, point)) return this.mode === "daily" ? this.selectStandard(this.gridSize, this.difficulty) : this.selectStandard(); if (this.renderer.hit(controls.leaderboard, point)) { const opened = this.leaderboard.openFriendBoard(); try { wx?.showToast?.({ title: opened ? "好友榜已请求加载" : "好友排名待微信授权", icon: "none" }); } catch (_) {} } return; }
