...
class ForestTrailMiniGame {
  ...
  start(level) {
    this.current = level; this.wasCompleted = false; this.dragCell = null; this.completionSummary = null; this.startedAt = Date.now(); this.renderer.setLevel(level); this.engine = new TrailEngine(level);
    this.unsubscribe?.(); this.unsubscribe = this.engine.subscribe((snapshot) => { if (snapshot.status === "completed" && !this.wasCompleted) { this.wasCompleted = true; this.completionSummary = this.progress.markComplete(level, { moves: snapshot.moves, elapsedMs: Date.now() - this.startedAt }); this.sound.complete(); } this.render(snapshot); });
  }
  view(snapshot = this.engine.getSnapshot()) { return { daily: this.mode === "daily", dailyComplete: this.mode === "daily" && this.progress.isDailyComplete(this.current), gridSize: this.gridSize, difficulty: this.difficulty, difficultyLabel: LABELS[this.difficulty], sound: this.progress.soundEnabled(), points: snapshot.status === "completed" ? Math.max(10, Math.round(1200 / Math.max(1, snapshot.moves))) : 0, time: formatTime(Date.now() - this.startedAt), completion: this.completionSummary || this.progress.getCompletionSummary(this.current), rankings: this.progress.rankingStatus() }; }
  ...
  handleStart(event) {
    ...
    if (this.engine.getSnapshot().status === "completed") { if (this.renderer.hit(controls.next, point)) return this.mode === "daily" ? this.selectStandard(this.gridSize, this.difficulty) : this.selectStandard(); if (this.renderer.hit(controls.leaderboard, point)) { try { wx?.showToast?.({ title: "好友排名待微信授权", icon: "none" }); } catch (_) {} } return; }
