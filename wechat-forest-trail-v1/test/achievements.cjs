...
// 1. 图鉴结构：首发 28 枚、8 个类别都有徽章、默认只开放铜/银、ID 唯一、每枚都有方向提示。
assert.strictEqual(BADGES.length, 28);
assert.strictEqual(new Set(BADGES.map((badge) => badge.id)).size, 28);
for (const badge of BADGES) {
  ...
  if (badge.id === "fruit-memory-master") {
    assert.ok(tiersAvailable(badge).every((step) => step.tier === "diamond"), "水果记忆大师应为钻石牌");
  } else {
...
for (const name of ["初次连线", "数字启程", "第一颗星", "开始记忆", "一笔不错", "完美棋盘", "快手连线", "闪电数字", "无误连线", "撤回绝缘体", "冷静到底", "速度大师", "淡影行者", "过目不忘", "闪现捕手", "短忆高手", "盲连专家", "记忆耐力", "连续专注", "记忆连胜", "一周不忘", "今日签到", "每日三星", "果园丰收", "星云航程", "主题收藏家", "水果记忆大师", "终极记忆者"]) assert.ok(BADGES.some((badge) => badge.name === name), `缺少首发徽章：${name}`);
...
const tap = (game, x, y) => game.handleStart({ touches: [{ clientX: x, clientY: y }] });
{
  ...
  assert.deepStrictEqual(game.view().badgeSummary, { unlocked: 0, total: 28, title: null });
  ...
  assert.strictEqual(game.view().badgeCollection.badges.length, 28);
...
// 7. 渲染器：成就按钮、图鉴弹窗三个页签、锁定/解锁图标、结算页成就行与“查看徽章”按钮。
{
  ...
  assert.ok(text.includes("🏅 成就 0/28"));
