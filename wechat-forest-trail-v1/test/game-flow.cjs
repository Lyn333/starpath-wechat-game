...
assert.equal(game.renderer.controls.volume.y, 68, "音量按键应位于音效按键下方");
...
const previousVolume = game.progress.soundVolume(); game.handleStart({ touches:[{ clientX: game.renderer.controls.volume.x + 2, clientY: game.renderer.controls.volume.y + 2 }] }); assert.notEqual(game.progress.soundVolume(), previousVolume, "点击音量按键应循环切换音量档位并保存");
