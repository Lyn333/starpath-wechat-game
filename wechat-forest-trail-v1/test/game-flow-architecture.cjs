...
resumed.progress.markFruitTutorialSeen();
resumed.challengePreviewUntil = Date.now() - 1;
resumed.fruitHideUntil = Date.now() - 1;
resumed.fruitConnectEndsAt = Date.now() + 120000;
resumed.fruitTutorialVisible = false;
for (const cell of resumed.current.solution) resumed.moveTo(cell);
