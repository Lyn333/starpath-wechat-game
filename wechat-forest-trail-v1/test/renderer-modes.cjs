...
const view = { mode: "standard", progressiveLevel: 1, difficulty: "easy", difficultyLabel: "简单", gridSize: "6x6", sound: true, points: 0, time: "0:00", clockActive: false, clockSetupVisible: false, clockEnded: false, clockTiers: Object.values(CLOCK_TIERS), infoVisible: true, themePickerVisible: false, rankings: { friend: { text: "好友榜" }, global: { text: "总榜" } }, completion: { best: { elapsedMs: 0 }, streak: 0 } };
renderer.render(snapshot, view);
assert.ok(renderer.controls.info && renderer.controls.theme && renderer.controls.selectedLevel && renderer.controls.sound && renderer.controls.daily && renderer.controls.progressive && renderer.controls.clock);
...
assert.ok(text.includes("渐进 Level 1"));
