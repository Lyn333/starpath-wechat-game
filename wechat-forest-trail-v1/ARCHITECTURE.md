...
参考游戏的 Beat the Clock 页面为不同档位提供不同的每局奖励时间，并让难度随波次升级。[3] 新版本已把时间奖励调整为 Easy +10 秒、Medium +7 秒、Hard +5 秒、Expert +3 秒；各档位可以在若干局后提升棋盘尺寸。Daily 使用 Asia/Shanghai 日期 Seed；关卡挑战提供水果乐园（10 秒记忆 + 全覆盖盲连）与太空旅行（点选连线）两套各 5 关；Unlimited 保留玩家自由选择的 6×6–12×12 与三档难度。
...
游戏控制区现已直接提供“每日挑战”和“🎯 关卡挑战”入口；限时弹窗展示每个档位的真实奖励秒数。Unlimited / Daily / Clock 复用同一个 `TrailEngine`（相邻拖动、覆盖全盘）。水果乐园改用 `FruitParadiseEngine`：10 秒记忆弹窗后隐藏全部水果与数字，按 1～10 检查点相邻盲连并覆盖 6×6 全盘，过程中不提示下一个目标。太空旅行仍用 `ChallengeEngine`（点选连线；第 3–5 关淡影/全藏/闪现）。数字连线进行中不再绘制下一个目标底圈。通关特效按主题切换为丰收落果或星云旋点。集齐主题关卡解锁奖励皮肤；通关丰收秘境另解锁丰收秘境背景，三星解锁彩虹水果路径。
