...
export async function buildMiniGamePackage({ destination = outputDir } = {}) {
  ...
  await writeFile(path.join(destination, "audio", "game.js"), "/** 微信普通分包入口：背景音乐资源。请勿使用 CommonJS 模块导出。 */\nconsole.log(\"森林寻径背景音乐分包已加载\");\n");
  ...
  await writeFile(path.join(destination, "catalog", "game.js"), "/** 微信普通分包入口：题库在主包请求加载完成后按需 require。请勿使用 CommonJS 模块导出。 */\nconsole.log(\"森林寻径首发题库分包已加载\");\n");
  ...
  await writeFile(path.join(destination, "release-manifest.json"), JSON.stringify({ ...metadata, packageSizeBytes: packageSize, appIdStatus: "configured-wx42d447652d8a5d07", audio: { backgroundMusic: { source: "user-provided-music", path: "audio/forest-trail-background.mp3", bytes: backgroundMusicBytes, loop: true }, numberEffect: { source: "modern-calendar-reminder-style-synthesis", trigger: "next-waypoint-only", notesHz: [1046.5, 1318.5, 1568] } } }, null, 2));
  console.log(`已生成微信小游戏发布包：${destination}\n主包代码已与 ${packageSize} 字节首发题库分离；已包含 ${backgroundMusicBytes} 字节循环背景音乐与现代日历提醒风格数字提示。`);
